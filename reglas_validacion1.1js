...
function validarMueble(card) {
            ...
            // Si ninguna pieza pudo compararse, recien ahi informa que faltan dimensiones utiles.
            if (filasValidadas === 0 && !ancho && !alto && !profundidad) {
                errores.push(`No he podido validar "${cod}" porque no pude detectar dimensiones suficientes desde el codigo del mueble.`);
            }
...
function validarMedidasPieza(pieza, medida1, medida2, modulo) {
            ...
            if (esPuerta(nombre)) {
                ...
                const cantidadPuertas = modulo.ancho > 700 ? 2 : 1;
                ...
                const altoPuerta = extraNovak ? modulo.alto + extraNovak : modulo.alto - 3;
            ...
            if (esOrejaAuxiliar(nombre)) {
                const anchosOreja = [60, 80, 100, 150];
                ...
                return {
                    ...
                    mensaje: alturasOreja.length
                        ? `deberia medir ${alturasOreja.join(" o ")} x 60/80/100/150 mm como oreja auxiliar segun pareja OTP/OA.`
                        : `deberia conservar ancho 60/80/100/150 mm como oreja auxiliar.`,
                    valida: true,
            ...
            if (esOrejaTipoPuerta(nombre)) {
                ...
                const anchosEstandarOreja = anchoOreja ? [anchoOreja] : [60, 80, 100, 150];
                ...
                return {
                    ok,
                    mensaje: alturasOreja.length
                        ? `deberia medir ${alturasOreja.join(" o ")} x 60/80/100/150 mm como oreja tipo puerta segun pareja OTP/OA.`
                        : `deberia conservar ancho 60/80/100/150 mm como oreja tipo puerta.`,
                    valida: true,
...
function analizarParejaOtpOa(card, alto, cod) {
            ...
            if (!emparejadas || !base) {
                return { emparejadas: false, descuentoEn: "", altoBase: base, altoConFuga: conFuga };
            }

            const otpConFuga = otp.every(item => coincideOreja(item, conFuga));
            const otpSinFuga = otp.every(item => coincideOreja(item, base));
            const oaConFuga = oa.every(item => coincideOreja(item, conFuga));
            const oaSinFuga = oa.every(item => coincideOreja(item, base));

            let descuentoEn = "";
            if (otpConFuga && oaSinFuga) descuentoEn = "OTP";
            else if (oaConFuga && otpSinFuga) descuentoEn = "OA";

            return { emparejadas: true, descuentoEn, altoBase: base, altoConFuga: conFuga };
        }

function coincideOreja(item, alto) {
            const anchos = [60, 80, 100, 150];
